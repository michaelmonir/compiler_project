...
int RulesParser::parseRegularDefinition(string token) {
    ...
    RegularDefinition regularDefinition(definition.first, definition.second);
    regularDefinition.formalizeRegex(regularDefinitions);
...
int RulesParser::parseRegularExpression(string token) {
    removeConsecutiveSpaces(token);
    pair<string, string> expression = splitToken(token, ':');
    trimString(expression.first);
    trimString(expression.second);
    // todo: need to creat the RegularExpression class
    // todo: need to add the definition to the regularExpressions vector
    RegularExpression regularExpression(expression.first, expression.second, regularExpressions.size() + REGULAR_EXPRESSION);
    regularExpression.formalizeRegex(regularDefinitions);
    regularExpressions.push_back(regularExpression);
    return 0;
}